...
const createStore = function(reducer, initState, rewriteCreateStore){
    ...
    if (rewriteCreateStore){
        var newCreateStore = rewriteCreateStore(createStore);
...
function applyMiddleware(...middlewares){
    return function(oldCreateStore){
        return function(reducer, initState){
            ...
            //相当于给每个middleware传store, 比如var loggerDis = loggerMiddleware(store);
            let mids = middlewares.map(middleware => middleware(store));
            let dispatch = store.dispatch; //最开始的dispatch
            mids.reverse().map((middleware)=>{
