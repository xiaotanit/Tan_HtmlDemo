...
Page({
  ...
  data: {
    ...
    loadCount: 0, //已经加载完成的图片数量
    lastTopImgIndex: 0, //上次计算的图片的索引
  ...
  loadImage: function (e) {
    ...
    var obj = this.data.dataList[index];
    ...
    this.setData({
      ...
      console.log('i = ' + index + ", imgH : " + imgScaleH + ", loadCount: " + that.data.loadCount + ", dataList.length: " + that.data.dataList.length)

      //判断在加载完最后一张图片后，计算每张图片的顶部位置
      if (that.data.loadCount == that.data.dataList.length) {
        that.setImageTopPosition();
      }
    });
  },
  //计算每张图片的top位置
  setImageTopPosition: function () {

    var dataList = this.data.dataList;
    var margin = this.data.imgMargin;  //图片间距
    var imgWidth = this.data.imgWidth; //图片宽度
    var lastIndex = this.data.lastTopImgIndex; //上次计算到哪张图片了
    //第一列的累积top，和第二列的累积top
    var firtColH = this.data.topArr[0], secondColH = this.data.topArr[1];

    for (var i = lastIndex; i < dataList.length; i++) {
      var obj = dataList[i];
